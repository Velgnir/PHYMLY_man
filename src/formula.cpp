...
double get_formula_result(const int i, const int j,
    ...
    } else if (i == static_cast<size_t>(height-1) && j== static_cast<size_t>(width-1)){
        return First_matrix[i][j] +
               (k1 * (First_matrix[i - 1][j] - First_matrix[i][j])) +
               (k2 * (First_matrix[i][j - 1] - First_matrix[i][j]));
    ...
    }else if (j == 0 && i == static_cast<size_t>(height-1)){
        return First_matrix[i][j] +
               (k1 * (First_matrix[i - 1][j] - First_matrix[i][j])) +
               (k2 * (First_matrix[i][j + 1] - First_matrix[i][j]));

    } else if (i == static_cast<size_t>(height-1)){
        return First_matrix[i][j] +
               (k1 * (First_matrix[i - 1][j] - First_matrix[i][j])) +
               (k2 * (First_matrix[i][j + 1] + First_matrix[i][j - 1] - 2 * First_matrix[i][j]));

    }else if (j == static_cast<size_t>(width-1)){
        return First_matrix[i][j] +
