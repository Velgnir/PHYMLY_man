...
class MainWindow(QMainWindow):
    ...
    def bp(self):
        ...
        try:
            ...
            path_to_matrix_file = self.ui.lineEdit_10.text()
            path_to_config_file = self.ui.lineEdit_9.text()
            coordinate_size_X = float(self.ui.lineEdit.text())
            coordinate_size_Y = float(self.ui.lineEdit_2.text())+1
            ...
            conf.read(path_to_config_file)

            conf.set("options", "temperature_limit", temperature_limit)
            conf.set("options", "step_in_X", step_in_X)
            conf.set("options", "step_in_time", step_in_time)
            conf.set("options", "step_in_Y", step_in_Y)
            conf.set("options", "heat_capacity", heat_capacity)


            with open(path_to_config_file, "w") as config:
                conf.write(config)
                    ...
                    for x in range(0,int(coordinate_size_X)):
                        ...
                        for y in range(0,int(coordinate_size_Y)):
                            if(eval(y_list[i][1::].replace("x",str(eval(x_string)))) == y):
                                matrix[y][x] = eval(temperature)
            matrix_file = open(path_to_matrix_file, "w")
            ...
            matrix_file.close()
            self.ui.label_18.setStyleSheet("color: rgba(255,0,0,0);")
            self.ui.label_11.setStyleSheet("color: rgba(255,0,0,0);")
            self.ui.label_10.setStyleSheet("color: rgba(255,0,0,0);")
            ...
            elif(os.path.isfile(path_to_config_file) != True or os.path.isfile(path_to_matrix_file) != True):
                self.ui.label_18.setStyleSheet("color: rgba(255,0,0,255);")
            else:
